...
export default function QuizApp() {
  ...
  const renderLoading = () => (
    <div className="flex flex-col items-center justify-center gap-4 text-center animate-in fade-in duration-500">
      ...
      <p className="text-muted-foreground">Please wait while we create questions on "{topic}".</p>
    </div>
  );
          ...
          <CardContent className="space-y-3">
            {currentQuestion.options.map((option, index) => (
              <Button
                ...
                variant="outline"
                className={cn(
                    'w-full h-auto justify-start text-left py-3 px-4 whitespace-normal transition-all duration-300',
            ...
            <CardContent className="flex flex-col items-center gap-4">
                <div className="relative flex items-center justify-center w-40 h-40">
                    <svg className="w-full h-full transform -rotate-90">
                        <circle className="text-secondary" strokeWidth="12" stroke="currentColor" fill="transparent" r="74" cx="80" cy="80" />
                        <circle
                          className="text-primary"
                          strokeWidth="12"
                          strokeDasharray={2 * Math.PI * 74}
                          strokeDashoffset={(2 * Math.PI * 74) * (1 - percentage / 100)}
                          strokeLinecap="round"
                          stroke="currentColor"
                          fill="transparent"
                          r="74"
                    ...
                    <span className="absolute text-4xl font-bold">{score}/{quizData!.questions.length}</span>
                </div>
                <p className="text-lg">You scored {percentage}% on the "{topic}" quiz.</p>
            </CardContent>
