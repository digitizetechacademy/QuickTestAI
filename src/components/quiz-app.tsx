        ...
        return (
          <Card className="w-full max-w-lg shadow-lg animate-in fade-in duration-500">
            <CardHeader className="text-center">
              ...
              <CardDescription>{t('topic_screen_description')}</CardDescription>
            </CardHeader>
            <CardContent>
              <Form {...form}>
                <form onSubmit={form.handleSubmit(handleTopicSubmit)} className="space-y-6">
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        <FormControl>
                          <Input placeholder={t('topic_placeholder')} {...field} className="text-center text-base" />
                        </FormControl>
                        <FormMessage />
                      </FormItem>
                    )}
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem className="space-y-3">
                        <FormLabel className="text-center block">{t('difficulty_select')}</FormLabel>
                        <FormControl>
                          <RadioGroup
                          ...
                          >
                            <FormItem className="flex items-center space-x-2 space-y-0">
                              ...
                              <FormLabel htmlFor="r1" className="font-normal text-base">{t('difficulty_easy')}</FormLabel>
                            </FormItem>
                            <FormItem className="flex items-center space-x-2 space-y-0">
                              <FormControl>
                                <RadioGroupItem value="Medium" id="r2" />
                              </FormControl>
                              <FormLabel htmlFor="r2" className="font-normal text-base">{t('difficulty_medium')}</FormLabel>
                            </FormItem>
                            <FormItem className="flex items-center space-x-2 space-y-0">
                              <FormControl>
                                <RadioGroupItem value="Hard" id="r3" />
                              </FormControl>
                              <FormLabel htmlFor="r3" className="font-normal text-base">{t('difficulty_hard')}</FormLabel>
                            </FormItem>
                          </RadioGroup>
                        </FormControl>
                        <FormMessage />
                  ...
                  <Button type="submit" className="w-full" disabled={form.formState.isSubmitting}>
                    {form.formState.isSubmitting ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : t('generate_quiz_button')}
                  </Button>
                </form>
              </Form>
            </CardContent>
      ...
      case 'loading':
        return (
          <div className="flex flex-col items-center justify-center gap-4 text-center animate-in fade-in duration-500">
            ...
            <h2 className="text-2xl font-semibold">{t('loading_title')}</h2>
            {quizParams && (
                <p className="text-muted-foreground max-w-sm">
                    {t('loading_description', { difficulty: t(`difficulty_${quizParams.difficulty.toLowerCase()}`), topic: quizParams.topic })}
                </p>
            )}
          </div>
        );
